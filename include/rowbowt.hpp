...
class RowBowt {
    ...
    */
    ...
    LFData& find_range_w_markers(const std::string query, uint64_t wsize, LFData& lf) const {
        lf.clear();
        if (!ma_) {
    ...
    LFData find_range_w_markers(const std::string query, uint64_t wsize) const {
        LFData lf;
        lf = find_range_w_markers(query, wsize, lf);
        return lf;
    }
